...
export class GroupController {
  ...
  async isMember(request: Request, response: Response): Promise<Response> {
    try {
      const groupId = request.params.id;
      const userId = request.params.userId;
      const isMember = await groupServiceInstance.isMember(
        Number(groupId),
        Number(userId)
      );
      return response.status(StatusCodes.OK).send(isMember);
    } catch (err) {
      return response
        .status(StatusCodes.INTERNAL_SERVER_ERROR)
        .json({ error: err instanceof Error ? err.message : 'Failed to do something exceptional' });
